...
const apiKeyLabel = apiKeyInput.closest("label");
const apiKeyStatus = document.createElement("span");
apiKeyStatus.className = "api-key-status";
apiKeyLabel.appendChild(apiKeyStatus);
...
function renderPresentTable(members) {
  ...
  presentMembers.innerHTML = `<table><thead><tr><th></th><th>ID</th><th>Name</th><th>Kontakt</th><th>NFC-IDs</th><th>Eingeloggt seit</th><th>Dauer</th><th>Aktion</th></tr></thead><tbody>${rows}</tbody></table>`;
...
function renderMemberTable(members) {
  ...
  memberList.innerHTML = `<table><thead><tr><th>ID</th><th>Name</th><th>Kontakt</th><th>Kategorie</th><th>Grundunterweisung</th><th>Anwesend</th><th>Letzter Besuch</th><th>Besuche</th><th>Anwesenheit</th><th>Team</th><th></th></tr></thead><tbody>${rows}</tbody></table>`;
...
// ===== NFC Scan History (client-side) =====
function updateNfcHistory(present, pending) {
  const container = document.querySelector("#nfcHistory");
  if (!container) return;
  const entries = [];

  for (const m of present) {
    if (m.current_login_at && m.nfc_ids?.[0]) {
      entries.push({
        nfc_id: m.nfc_ids[0],
        name: memberName(m),
        action: "check-in",
        time: m.current_login_at,
      });
    }
  }
  ...
  const countEl = document.querySelector("#nfcHistoryCount");
  ...
  container.innerHTML = `<table><thead><tr><th>NFC-ID</th><th>Name</th><th>Aktion</th><th>Zeit</th></tr></thead><tbody>${entries
    .slice(0, 5)
    .map(
      (e) =>
        `<tr><td>${e.nfc_id}</td><td>${e.name}</td><td>${e.action}</td><td>${formatDate(e.time)}</td></tr>`,
    )
    .join("")}</tbody></table>`;
}
...
async function loadData() {
  ...
  updateNfcHistory(present, pending);
}
